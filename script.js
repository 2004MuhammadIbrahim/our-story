document.addEventListener("DOMContentLoaded", function() {
    ...
    const animatedElements = document.querySelectorAll('.fade-in, .slide-in-left, .slide-in-right, .pop-in');
...
function scrollToTimeline() {
  ...
  if (music) { music.play().catch(error => console.log("Audio block")); }
...
function showFinalMessage() {
  ...
  if (finalMessage) {
    ...
    if (music) { music.play().catch(e => console.log("Audio block")); }
    setTimeout(() => { window.scrollTo({ top: document.body.scrollHeight, behavior: "smooth" }); }, 100);
  }

  if (typeof confetti !== "undefined") {
    confetti({ particleCount: 300, spread: 180, origin: { y: 0.5 }, colors: ['#ff4d6d', '#ff87ab', '#ffffff'] });
  }
}